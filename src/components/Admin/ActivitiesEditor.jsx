...
import { getActivities, addActivity, updateActivity, deleteActivity, updateActivityOrder, timestampToDateString } from '../../services/dataService';
...
function ActivitiesEditor() {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      setError('Failed to load activities items: ' + err.message);
      setItems([]);
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editing) {
        await updateActivity(editing.id, dataToSave);
      } else {
        await addActivity(dataToSave);
      }
  ...
  return (
    <div className="editor-container">
      <h2>Manage Co-curricular Activities</h2>
      ...
      <div className="items-list">
        ...
        {loadingItems ? (
          ...
          <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
            No items yet. Add your first activity entry above.
          </div>
        ) : (
          items.map((item, index) => (
            ...
            >
              ...
              >☰</span>
              <h4>{item.organization} - {item.role}</h4>
