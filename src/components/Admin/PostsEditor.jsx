...
import { getPosts, addPost, updatePost, deletePost, updatePostOrder, timestampToDateString } from '../../services/dataService';
...
function PostsEditor() {
  ...
  const [formData, setFormData] = useState({ title: '', date: '', description: '', content: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      if (editing) await updatePost(editing.id, formData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Delete this post?')) {
      try { await deletePost(id); await loadItems(); } catch (error) { alert('Error deleting'); }
    }
  };

  const resetForm = () => { setEditing(null); setFormData({ title: '', date: '', description: '', content: '' }); };
  ...
  const handleDrop = async (e) => {
    ...
    setDragOverIndex(null);
    try {
      await Promise.all(Object.entries(updates).map(([id, order]) => updatePostOrder(id, order)));
      await loadItems();
  ...
  return (
    <div className="editor-container">
      ...
      <form onSubmit={handleSubmit} className="editor-form">
        ...
        <div className="form-group"><label>Content (Markdown with LaTeX supported)</label><textarea value={formData.content} onChange={(e) => setFormData({ ...formData, content: e.target.value })} rows="10" placeholder="Write your post content in Markdown. Use $$ for LaTeX equations." /></div>
        <button type="submit" disabled={loading} className="btn btn-primary">{loading ? 'Saving...' : editing ? 'Update' : 'Add Post'}</button>
      ...
      <div className="items-list">
        ...
        {loadingItems ? <div style={{ padding: '2rem', textAlign: 'center' }}>Loading...</div> :
          items.length === 0 ? <div style={{ padding: '2rem', textAlign: 'center' }}>No posts yet.</div> :
