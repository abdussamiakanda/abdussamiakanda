...
import { getPublications, addPublication, updatePublication, deletePublication, updatePublicationOrder } from '../../services/dataService';
...
function PublicationsEditor() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Saving publication data:', dataToSave);

      if (editing) {
        await updatePublication(editing.id, dataToSave);
      } else {
    ...
    try {
      await Promise.all(
        Object.entries(updates).map(([id, order]) => 
          updatePublicationOrder(id, order)
        )
      );
      await loadItems();
    } catch (error) {
    ...
    <div className="editor-container">
      ...
      <div className="items-list">
        ...
        {loadingItems ? (
          ...
          <div style={{ padding: '2rem', textAlign: 'center', color: 'var(--text-secondary)' }}>
            No publications yet. Add your first publication above.
          </div>
        ) : (
