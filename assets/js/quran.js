document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchVerses(reference) {
        // Check if it's a range (e.g., "1:1-3")
        const rangeMatch = reference.match(/(\d+):(\d+)-(\d+)/);
        const cacheKey = reference;
        
        if (rangeMatch) {
            const [_, surah, startVerse, endVerse] = rangeMatch;
            const verses = [];
            
            for (let verse = parseInt(startVerse); verse <= parseInt(endVerse); verse++) {
                const singleRef = `${surah}:${verse}`;
                
                if (verseCache.has(singleRef)) {
                    verses.push(verseCache.get(singleRef));
                    continue;
                }

                try {
                    const [arResponse, enResponse] = await Promise.all([
                        fetch(`https://api.alquran.cloud/v1/ayah/${singleRef}/ar`),
                        fetch(`https://api.alquran.cloud/v1/ayah/${singleRef}/en.sahih`)
                    ]);
                    ...
                    if (arData.code === 200 && enData.code === 200) {
                        ...
                        verseCache.set(singleRef, verseData);
            ...
            return {
                verses,
                surahName: verses[0]?.surahName || 'Unknown Surah'
            };
        } else {
            // Handle single verse
            if (verseCache.has(cacheKey)) {
                return {
                    verses: [verseCache.get(cacheKey)],
                    surahName: verseCache.get(cacheKey).surahName
                };
            }

            try {
                const [arResponse, enResponse] = await Promise.all([
                    fetch(`https://api.alquran.cloud/v1/ayah/${reference}/ar`),
                    fetch(`https://api.alquran.cloud/v1/ayah/${reference}/en.sahih`)
                ]);
                ...
                return {
                    verses: [{
                        arabic: `[Error loading verse ${reference}]`,
                        text: `[Error loading verse ${reference}]`,
                        surahName: 'Error'
                    }],
                    surahName: 'Error'
                };
    ...
    function createVersePreview(reference, verseData) {
        const arabicText = verseData.verses.map(verse => verse.arabic).join(' ');
        const translatedText = verseData.verses.map(verse => verse.text).join(' ');

        return `<div class="verse-preview" contenteditable="false" data-verse-ref="${reference}">
            <div class="verse-arabic">${arabicText}</div>
            <div class="verse-text">${translatedText}</div>
            <div class="verse-reference">${verseData.surahName} (${reference})</div>
        </div>`;
    }
    ...
    function cleanContent(content) {
        ...
        const previews = tempDiv.querySelectorAll('.verse-preview');
    ...
    window.showEditor = async function() {
        ...
        if (!editorView.dataset.editingPostId) {
            ...
            // Process verse tags for existing content
            const versePattern = /v\[(\d+:\d+(?:-\d+)?)\]/g;
            const matches = [...content.matchAll(versePattern)];
            ...
            for (const match of matches) {
                ...
                try {
                    ...
                    editor.innerHTML = editor.innerHTML.replace(fullMatch, fullMatch + preview);
                } catch (error) {
                    console.error('Error fetching verse:', error);
                }
            }
    ...
    // Make sure initEditor only handles new changes
    function initEditor() {
        const editor = document.getElementById('postContent');
        let lastContent = editor.dataset.lastContent || editor.innerHTML;
        let editorTimeout;
        ...
        editor.addEventListener('input', function() {
            ...
            editorTimeout = setTimeout(async () => {
                if (content.includes('v[')) {
                    ...
                    for (const match of matches) {
                        const [fullMatch, reference] = match;
                        const existingPreview = editor.querySelector(
                            `.verse-preview[data-verse-ref="${reference}"]`
                        );
                ...
                const previews = editor.querySelectorAll('.verse-preview');
                previews.forEach(preview => {
                    const reference = preview.dataset.verseRef;
                    if (!content.includes(`v[${reference}]`)) {
                        preview.remove();
                    }
                });
                
                lastContent = editor.innerHTML;
                editor.dataset.lastContent = lastContent;
            }, 500);
        });
    }
    ...
    window.loadPosts = function() {
        ...
        postsRef.on('value', (snapshot) => {
            ...
            postsArray.forEach(post => {
                if (post.isPublic || post.authorId === auth.currentUser?.uid) {
                    displayPost(post);
    ...
    window.displayPost = async function(post) {
        ...
        const contentPreview = tempDiv.textContent.slice(0, 10) + 
    ...
    window.editPost = async function(postId) {
        const postRef = database.ref('posts').child(postId);
    ...
    window.publishPost = async function() {
        ...
        try {
            ...
            if (editingPostId) {
                ...
                postRef = database.ref('posts/' + editingPostId);
