  ...
  document.getElementById('top').innerHTML = `
    <div class="top-flex">
      <div class="top-flex-left">
        ...
        <div class="title" onclick="showAll()">QURAN</div>
      </div>
...
function addLog() {
  ...
  if (title && details && tags) {
    ...
    if (public === 'true') {
      database.ref("/qpublic/" + lid).set(true);
    }
    showThings('single');
    showSingle(lid);
...
function showMain() {
  ...
  database.ref("/quran").orderByKey().limitToLast(50).once("value").then((snap) => {
...
function delvasp(key) {
  database.ref('/quran/'+key).remove();
  database.ref('/qpublic/'+key).remove();
...
function showEditBox(key) {
  database.ref("/quran/"+key).once("value").then((snap) => {
    var title = snap.child("title").val();
    var details = snap.child("details").val();
    var tags = snap.child("tags").val();
    var public = snap.child("qpublic").val();
...
function editEntry(key) {
  ...
  if (title && details && tags) {
    ...
    if (public === 'true') {
      database.ref("/qpublic/" + key).set(true);
...
function showSingle(id) {
  database.ref("/quran/"+id).once("value").then((snap) => {
    var title = snap.child("title").val();
    var details = snap.child("details").val();
    var tags = snap.child("tags").val();
    tags = tags.replaceAll(',','</span><span>')
...
function makePin(id,tag) {
  if (tag === 'yes') {
    database.ref("/quran/"+id+"/pin").set('yes');
  } else {
    database.ref("/quran/"+id+"/pin").set('no');
  }
  showSingle(id);
}
...
function showSearchResult() {
  ...
  database.ref("/quran").orderByKey().once("value").then((snap) => {
    snap.forEach(function (childSnap) {
      var title = snap.child(childSnap.key + "/title").val();
